...
function copyShippingAddressToShipment(shippingData, shipmentOrNull) {
    ...
    Transaction.wrap(function () {
        ...
        var kamalCustomValue = shippingData.address.kamalCustom;
        dw.system.Logger.warn('KamalCustomValue  =  '+kamalCustomValue);
        shippingAddress.getCustom().kamalCustom = shippingData.address.kamalCustom;
...
function placeOrder(order, fraudDetectionStatus) {
    ...
    try {
        ...
        order.getCustom().Product_Name = 'hdjhjjsdhjds';
        Transaction.commit();
    } catch (e) {
        Transaction.wrap(function () { OrderMgr.failOrder(order, true); });
        result.error = true;
